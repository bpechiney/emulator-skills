...
- **[emulator-code-review](./skills/engineering/emulator-code-review/SKILL.md)** — Reviews Zig code for cycle-accurate Game Boy, NES, and SNES emulators. Per-component checklists (CPU, PPU, APU, bus, mapper, save-state, cartridge loader, test-ROM harness), cycle-accuracy bug catalogue, symptom→cause routing, citation rubric (✅⚠️❌), confidence-with-test-ROM-evidence output. Pairs with `/emudev` as the review-side counterpart — `/emudev` writes the code; this skill reviews it.
