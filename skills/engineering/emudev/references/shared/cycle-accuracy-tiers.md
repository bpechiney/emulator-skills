...
- **Pros**: Simple to implement. Fast. CPU loop is a `while` over decode + execute. Acceptable where the PPU is loosely coupled to the CPU.
- **Cons**: Cannot reproduce mid-instruction events. PPU/CPU bus contention is invisible. DMC sample-stealing CPU stalls (NES) cannot be modeled. Sprite-0 hit (NES) timing approximations only. Mode-3 length variance (Game Boy) impossible.
...
- **Pros**: Sufficient for most Game Boy correctness. Reasonable performance. Suitable middle ground.
...
- **Pros**: Reproduces every quirk the cycle-accuracy tier can in principle reproduce. Pixel-precise PPU (mode-3 length variance, mid-scanline writes), bus contention, DMC sample-stealing.
